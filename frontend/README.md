...
- **React 19** + **react-router-dom** (routing, and its data APIs: `loader`/`action` per route)
...
- `FindingFields` — the finding form fields, shared by the new-finding and edit forms
...
Pages are route `lazy` modules, so the heavy editor/mermaid chunks (and each
page's own data loading) only download when you open them.

## Layout

```
src/
  index.tsx              entry: resolves the auth mode, then mounts the router
  ...
  auth/session.ts        auth mode + token storage + the root loader
  App.tsx                route tree, header, error element
  pages/                 each exports Component + loader/action
    LoginPage.tsx  RegisterPage.tsx   (jwt mode only)
