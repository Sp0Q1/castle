...
#
# Needs: kind, kubectl, helm, and docker OR podman. No cloud, no DNS, no certs
# (routing is tested over HTTP via nip.io + Host headers). Run from the repo root.
# ---------------------------------------------------------------------------
set -uo pipefail
...
deploy(){  # $1=release $2=namespace $3=codename
  helm upgrade --install "$1" deploy/k8s/tenant \
    ...
    --set image.repository=castle --set image.tag=local --set image.pullPolicy=IfNotPresent \
...
for cn in ironclad thunderbolt nightfall; do
  code=$(curl -s -o /dev/null -w '%{http_code}' --max-time 8 -H "Host: $(H $cn)" "http://127.0.0.1:${HTTP_PORT}/probe-$cn")
  echo "  $(H $cn)/probe-$cn -> HTTP $code"
done
...
echo -n "  ironclad -> thunderbolt:5150  : "; conn tenant-ironclad castle-ironclad "$IPT" && echo "CONNECTED (unexpected!)" || echo "blocked ✓"
echo -n "  ironclad -> cluster DNS (allowed): "; kubectl -n tenant-ironclad exec deploy/castle-ironclad -- timeout 5 getent hosts kubernetes.default.svc.cluster.local >/dev/null 2>&1 && echo "resolves ✓" || echo "FAILED"
...
EOF
sleep 4
echo -n "  ironclad -> thunderbolt:5150 (now): "; conn tenant-ironclad castle-ironclad "$IPT" && echo "CONNECTED ✓ (policy was the enforcer)" || echo "still blocked (unexpected)"
...
n=$(kubectl -n ingress-nginx logs deploy/ingress-nginx-controller 2>/dev/null | grep -c 'probe-nightfall')
echo "  ingress-logged requests to decoy 'nightfall': $n"

step "done — explore it yourself"
cat <<EOF
  curl -H 'Host: ironclad.127.0.0.1.nip.io'    http://127.0.0.1:${HTTP_PORT}/
  curl -H 'Host: nightfall.127.0.0.1.nip.io'   http://127.0.0.1:${HTTP_PORT}/    # decoy
